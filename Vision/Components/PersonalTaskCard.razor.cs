...
{
    public partial class PersonalTaskCard
    {
        [Parameter]
        ...
        public void ActiveBillingCode()
        {
            ...
            timeEntryService.UpdateActiveTimeEntry(Item.BillingCodeId ?? 0, TimerActive, Item.Title, TimerActive.ToString(), Item.StepType, Item.WorkflowStepId);
            StateHasChanged();
        }
        private void OpenAutotask()
        {
            ...
            if (Item.StepType == 0)
                {
                    JSRuntime.InvokeVoidAsync("OpenUrlInNewTab", url += $"Projects/TaskDetail.mvc?taskId={Item.WorkflowStepId}");
                }
                else
                {
                    JSRuntime.InvokeVoidAsync("OpenUrlInNewTab", url += $"ServiceDesk/TicketDetail.mvc?ticketId={Item.WorkflowStepId}");
                }       
        }
        ...
        /// </summary>
        /// <param name="value"></param>
        private async Task SecondaryValuesChanged(IEnumerable<long?> value)
        {
